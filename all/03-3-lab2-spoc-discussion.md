...
1. X86有几个特权级？
   - 4个特权级，通常操作系统只适用2个，0和3。


2. 不同特权级有什么区别？
   - 特权指令只能在ring 0使用；
   - 一条指令在不同特权下能访问的数据范围不同；
   - 一条指令在不同特权级下行为不同。
...
1. 一条指令在执行时会有哪些可能的特权级判断？
   - 当代码访问数据时，需要检查当前特权级是否高于或等于要访问的数据段的DPL；
   - 如果是通过段寄存器SS访问数据段，则要求CPL、RPL = DPL；
   - 如果代码涉及到控制转移，则一般只允许第特权级调用高特权级代码；
...
3. int指令在ring0和ring3的执行行为有什么不同？
   - 进行了栈切换
   - 压栈内容不同，多了一个SS:ESP的压栈


4. 如何利用int和iret指令完成不同特权级的切换？
   - 人工构造所需要的栈结构，然后通过int和iret指令进行切换，


5. TSS和Task Register的作用是什么？
   - TSS（Task State Segment）中存放不同特权级的SS和ESP（堆栈信息）
   - Task Register存放TSS的段描述信息，便于快速访问。
...
2. 描述X86-32的MMU地址转换过程；
   - 从虚拟地址通过段基址转换成线性地址，再通过页机制从线性地址转为物理地址。
