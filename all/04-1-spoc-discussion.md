...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   - 覆盖技术：依据程序逻辑结构，将程序划分为若干功能相对独立的模块；将不会同时执行的模块共享同一块内存区域。
   - 主要工作：模块划分、确定模块的共享关系。
2. 什么是交换技术？覆盖与交换有什么不同？
   - 交换技术：作为一个整体的进程地址空间之间的换入和换出
   - 不同：
     - 覆盖：发生在运行程序的内部模块间，通过切块，解决进行空间大于物理内存的问题
     - 交换：发生在内存进程间，通过进程整体交换，解决高优先级进程运行需要的内存空间不够问题；
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   - 空间局部性：如果特定存储位置在特定时间被访问，则很可能在不久的将来访问附近存储位置。
...
### 8.5 虚拟页式存储
 ...
 1. 为了支持虚拟页式存储的实现，页表项有什么修改？
  ...
  2. 页式存储和虚拟页式存储的区别是什么？
     - 虚拟页式存储，逻辑地址经过页表转换得到的物理地址不一定在内存中，需要从外存中进行加载。
